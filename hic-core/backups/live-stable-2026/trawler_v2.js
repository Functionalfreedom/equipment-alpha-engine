...
async function runTrawler() {
    ...
    try {
        console.log("[HIC] 2026 LIVE SYNC: Deep-Scanning McDougall...");
        ...
        await page.evaluate(async () => { window.scrollBy(0, 1500); await new Promise(r => setTimeout(r, 2000)); });
        ...
        const assets = await page.evaluate(() => {
            ...
            return blocks.map(el => {
                ...
                return { title, bid: bidMatch ? bidMatch[0].replace(/[^0-9.]/g, '') : "0", url: link, expiry: expiryMatch ? expiryMatch[0] : "Check Site", lot: lotMatch ? lotMatch[1] : "N/A" };
            }).filter(a => a.bid !== "0");
        });

        console.log(`[HIC] Processing ${assets.length} Units...`);
        ...
        for (let a of assets) {
            ...
            await pool.query(`
                ...
                ON CONFLICT (uid) DO UPDATE SET current_bid = EXCLUDED.current_bid, spread_value = EXCLUDED.spread_value, hic_fee = EXCLUDED.hic_fee, updated_at = NOW()
            `, [uid, a.title, parseFloat(a.bid), fmv, spread, fee, a.url, a.expiry]);
        }
        console.log("[SUCCESS] 34 Assets Pushed to Hierarchy Core.");
