    ...
    try {
        ...
        let rows = result.rows.map(i => {
            ...
            <tr style="border-bottom: 1px solid #eee; font-family: sans-serif;">
                ...
                <td style="padding:15px;">
                    <button style="background:#1a202c; color:white; border:none; padding:8px 15px; border-radius:4px; cursor:pointer; font-weight:bold;">
                        GENERATE PITCH
                    </button>
                </td>
            </tr>`;
        ...
        res.send(`
        <html>
            <body style="background:#f7fafc; padding:40px; font-family: sans-serif;">
                <h1 style="color:#1a202c;">HIERARCHY INVESTMENT CAPITAL</h1>
                <table style="width:100%; background:white; border-collapse:collapse; border-radius:8px; overflow:hidden; box-shadow:0 4px 6px rgba(0,0,0,0.1);">
                    <tr style="background:#1a202c; color:white; text-align:left;">
                        <th style="padding:15px;">ASSET</th><th style="padding:15px;">CURRENT BID</th><th style="padding:15px;">HIC FEE</th><th style="padding:15px;">ACTION</th>
                    </tr>
                    ${rows}
                </table>
            </body>
...
app.listen(8080, () => console.log('[DASHBOARD] Live at http://localhost:8080'));
